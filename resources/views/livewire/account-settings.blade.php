<section>
    ...
    <div class="grid grid-cols-1 gap-6 mt-6">
        ...
        <flux:fieldset class="p-6 border shadow-sm rounded-md">
            ...
            <form wire:submit.prevent="updateProfileInformation" class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <flux:input :label="__('Email')" wire:model="email" type="email" required autocomplete="email" />
                </div>
        ...
        <!-- Manage Companies Fieldset -->
        <flux:fieldset class="p-6 border shadow-sm rounded-md">
            <h3 class="text-xl font-bold text-gray-900 dark:text-gray-100 mb-4">
                {{ __('Manage Companies') }}
            </h3>
            <p class="text-sm text-gray-600 dark:text-gray-300 mb-6">
                {{ __('View, edit or delete your companies. Click "Add Companies" to add a new one.') }}
            </p>
                    ...
                    <flux:card class="rounded-lg border flex flex-col justify-between">
                        @if ($editingCompanyId === $company->id)
                            ...
                            <div class="space-y-3">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-sm mt-2">{{ __('Click to upload a new logo.') }}</p>
                                </div>
                        ...
                        @else
                            ...
                            <div class="mt-4 flex justify-end gap-2">
                                ...
                                <flux:button wire:click="deleteCompany({{ $company->id }})" variant="danger">
                                    Delete
                                </flux:button>
                            </div>
                        @endif
        ...
        <flux:modal name="delete-account-modal" class="md:w-96 space-y-6">
            <flux:heading size="lg">{{ __('Confirm Account Deletion') }}</flux:heading>
            <p class="text-sm">
                {{ __('Please enter your password to confirm that you want to delete your account.') }}
            </p>
            <flux:input :label="__('Password')" wire:model.defer="deleteAccountPassword" type="password" required />
            <div class="flex justify-end gap-4 mt-4">
                <flux:button wire:click="cancelDeleteAccount">{{ __('Cancel') }}</flux:button>
                <flux:button type="button" wire:click="confirmDeleteAccount" variant="danger">
                    {{ __('Delete Account') }}
                </flux:button>
            </div>
        </flux:modal>
